...
from models.manager import get_model2
import numpy as np
import math
...
def main(args):
	...
	xp = Backend(args)
	m = get_model2(args,W1,W2)
	opt = get_opt(args)
	if(loadflag == True):
		serializers.load_npz('./savedWN/ModelA3CNN_ATT_0817_tail.model',m)
	opt.setup(m)
	...
	for epoch in range(args.epoch_size):
		...
		normalizeR = m.Normal(xp)
		tool.trace('epoch:',epoch+59,'tr Loss:',tool.dress(trLoss),Ntr)
		if(epoch % 5 == 0):
			serializers.save_npz('./savedWN/ModelA3CNN_ATT_0817_tail.model',m)
...
def argument():
	...
	p.add_argument('--pretrain_file', '-ptF', default='pretrainModel/WN11-tail3000')

	# trian, dev, test, and other filds
	p.add_argument('--train_file',      '-tF',  default='datasets/OOKB/tail-3000/train')
	p.add_argument('--dev_file',        '-vF',  default='datasets/OOKB/tail-3000/dev')
	p.add_argument('--test_file',       '-eF',  default='datasets/OOKB/tail-3000/test')
	p.add_argument('--auxiliary_file',  '-aF',  default='datasets/OOKB/tail-3000/aux')
