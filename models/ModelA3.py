...
class Tunnel(chainer.Chain):
	...
	"""
	def __call__(self,x,rel_y,neighbor_entities,neighbor_dict,assign,entities,relations,RC,EC):
		if self.layer==0:
		...
		for r in bundle:
			...
			if len(rx)==1:
				rx=rx[0]
				if r%2==0:	rx = getattr(self,self.forwardH[0][0])(F.concat((rx,r_rep),axis=1))
				else:		rx = getattr(self,self.forwardT[0][0])(F.concat((rx,r_rep),axis=1))
				result[assignR[(r,0)]] = rx
			else:
				size = len(rx)
				#tempRx = list()
				for i in range(size):
					rx[i] = F.concat((rx[i],r_rep),axis=1)
				rx = F.concat(rx,axis=0)
				if r%2==0:	rx = getattr(self,self.forwardH[0][0])(rx)
				else:		rx = getattr(self,self.forwardT[0][0])(rx)
...
class Model(chainer.Chain):
	...
	def get_context(self,entities,links,relations,edges,order,xp,RC,EC):
		...
		x = getattr(self,self.forwardB[order][0])(x,rel_y,neighbor,neighbor_dict,assign,entities,relations,RC,EC)
		return x
