...
def main(args):
	...
	opt = get_opt(args)
	if(loadflag == True):
		serializers.load_npz('./savedWN/ModelA0_OOKBtest_'+args.pooling_method,m)
	opt.setup(m)
	for epoch in range(args.epoch_size):
		...
		if(epoch%10 == 0):
			serializers.save_npz('./savedWN/ModelA0_OOKBtest_'+args.pooling_method,m)
		dump_current_scores_of_devtest(args,m,xp)
...
def argument():
	...
	p.add_argument('--entity_size', '-Es',      default=12104,		type=int)
	...
	# sizes
	p.add_argument('--train_size',  	'-trS',  default=150000,       type=int)
