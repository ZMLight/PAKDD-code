...
def main(args):
	...
	opt = get_opt(args)
	if(loadflag == True):
		serializers.load_npz('./savedWN/ModelA3CNN_OOKBtest_'+args.pooling_method+'.model',m)
	opt.setup(m)
	...
	for epoch in range(args.epoch_size):
		...
		if(epoch % 5 == 0):
			serializers.save_npz('./savedWN/ModelA3CNN_OOKBtest_'+args.pooling_method+'.model',m)
...
def argument():
	...
	p.add_argument('--gpu_device',  '-gd',  default=7,      type=int)

	# trian, dev, test, and other filds
	p.add_argument('--train_file',      '-tF',  default='datasets/OOKB/head-3000/train')
	p.add_argument('--dev_file',        '-vF',  default='datasets/OOKB/head-3000/dev')
	p.add_argument('--test_file',       '-eF',  default='datasets/OOKB/head-3000/test')
	p.add_argument('--auxiliary_file',  '-aF',  default='datasets/OOKB/head-3000/aux')
	...
	p.add_argument('--rel_size',  	'-Rs',      default=836,		type=int)
	p.add_argument('--entity_size', '-Es',      default=10228,		type=int)

	# model parameters (neural network)
	p.add_argument('--nn_model',		'-nn',  default='A3_CNN')
